...
def exists():
    ...
    with check_jupyter.executor() as execute:
        for cell in cells:
            ...
            # If it's not a test cell, but it failed, record result for next test cell
            elif not passed:
                remaining_cells = cells[cells.index(cell) + 1:]
                for other_cell in remaining_cells:
                    if check_jupyter.is_test_cell(other_cell):
                        results.append((check_jupyter.get_cell_id(other_cell), passed, exception))

            # If cell did not pass, stop
            if not passed:
                break
